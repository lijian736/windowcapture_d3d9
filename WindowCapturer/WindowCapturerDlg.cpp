...
void CWindowCapturerDlg::on_capture(unsigned char* data, unsigned long len,
	unsigned int renderWidth, unsigned int renderHeight)
{
	...
	bmi.bmiHeader.biHeight = -(LONG)renderHeight;
	bmi.bmiHeader.biPlanes = 1;
	bmi.bmiHeader.biBitCount = 32;
	bmi.bmiHeader.biCompression = BI_RGB;

	StretchDIBits(dc, dstX, dstY, dstwidth, dstheight, 0, 0, renderWidth, renderHeight, data, &bmi, DIB_RGB_COLORS, SRCCOPY);
}
...
{
	D3D9CaptureScreen* capture = new (std::nothrow) D3D9CaptureScreen();
	if (!capture)
	{
		return;
	}
	...
	capture->set_draw_callback(std::bind(&CWindowCapturerDlg::on_capture, this,
